...
from base_player_byoyomi10000 import BasePlayer
...
class MCTSPlayer(BasePlayer):
    # USIエンジンの名前
    name = 'AI_3' #(c_puct, 近づける値との勝率差, 該当なしの場合に優先する値, 近づける値)
    # デフォルトチェックポイント
    DEFAULT_MODELFILE = '/content/Rival_flood/GCTdenryu_0001-035.pth'
    ...
    def go(self):
        ...
        if self.debug:
            for i in range(len(current_node.child_move)):
                print('{:3}:{:5} move_count:{:4} value_rate:{:.5f} policy_rate:{:.5f} win_rate:{:.5f}'.format(
    ...
    def get_bestmove_and_print_pv(self):
        ...
        #print(f'player_win_rate\n{player_value_list}\n')
        player_value_ave = mean(player_value_list)
        ...
        #print(f'win_sort_idx\n{win_sort_idx}\n')

        #print(player_value_ave)
        win_rate_diff = np.abs(player_value_ave - win_rate)
        #print(f'\nwin_rate_diff\n{win_rate_diff}\n')
        #print(current_node.policy)
        #print()
        ...
        for i in win_sort_idx:
            ...
            if win_rate_diff[i] <= 0.1:
                #print(win_rate_diff[i], current_node.policy[i], i)
                if current_node.policy[i] > current_node.policy[selected_index]:
                    #print(current_node.policy[i], current_node.policy[selected_index])
                    selected_index = i
    ...
    def check_interruption(self):
        ...
        (first < second * 1.5 or current_node.child_sum_value[first_index] / child_move_count[first_index] < current_node.child_sum_value[second_index] / child_move_count[second_index]):
            # 探索時間を2倍に延長
            self.time_limit *= 2
            # 探索延長は1回のみ
